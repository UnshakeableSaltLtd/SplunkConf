...
in the loop. It builds directly on [`TA_ResponseActions`](../../apps/TA_ResponseActions/README.md)
...
3. **The `notable_slack_enrichment` KV store + custom drilldown**
   ...
   [`TA_ResponseActions`'s README](../../apps/TA_ResponseActions/README.md#closing-the-loop-getting-the-result-back-to-the-analyst).
...
The full retired script and its `inputs.conf` registration are not included
in this public repository (they referenced an internal-only hostname); this
write-up is the retained record of that attempt, kept as a "before"
...
| — | Slack → HEC bridge (async verdict → CIM Risk fields via HEC) | Retired (shipped v1.3.1–v1.3.2, removed in v1.3.3) — not included in this public repository |
