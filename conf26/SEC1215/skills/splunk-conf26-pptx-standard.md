...
## Image Sourcing & Attribution Rules

These rules apply to every image used in any slide produced under this standard — headshots, externally sourced stock imagery, and AI-generated images.

**General rules:**
- Always credit the original source when using an externally sourced image.
- Never use imagery that includes visible watermarks, logos, or restrictive usage terms — check licensing before selecting an image, not after.
- Footnote text should be set very small (well below body copy size — treat it like fine print) so it never competes with or detracts from the slide's main message.

**Per-image-type footnote wording (on-slide, small text):**

| Image type | Footnote wording |
|---|---|
| David Pollard's own headshot / self-introduction photo | "Image provided by Unshakeable Salt Ltd for free distribution under commons licensing." |
| External stock image (e.g. Pixabay) | State the image's full source (e.g. "Image: [description], Pixabay.com") |
| Perplexity-generated image | "Images generated by Perplexity AI." |

**Mandatory speaker-notes attribution block:**

Every slide's speaker notes must end with this exact block (adjust the Pixabay image list to whatever images actually appear on that specific slide; leave the list section present but empty if no Pixabay images are used on that slide):

```
#######################################
Sources and Attribution

As a Director of Unshakeable Salt Ltd, David Pollard is authorised to and makes free any photos or images owned, produced and distributed within this slide deck.

Images sourced from Pixaby.com
<list images>

AI Generation:
Images, Text and Code throughout this slide deck have been generated or improved by the toolsets provided by 'Perplexity', 'Grammarly' and within GitHub by CoPilot.
#######################################
```

This block is boilerplate and must appear on every slide's notes, appended after any slide-specific speaker notes content (e.g. the CAF/NIST framework tie-ins already used on the opening quote slide) — never in place of it.
